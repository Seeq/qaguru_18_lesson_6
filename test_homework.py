...
def test_dark_theme_by_time():
    ...
    if current_time >= time_evening or current_time <= time_morning:
        is_dark_theme = True
    else:
        is_dark_theme = False
...
def function_beautify(func, *args):
    function_name = func.__name__.replace('_', ' ').title()
    arguments_name = ", ".join(args)
...
def open_browser(browser_name):
    actual_result = function_beautify(open_browser, browser_name)
    assert actual_result == "Open Browser [Chrome]"


def go_to_companyname_homepage(page_url):
    actual_result = function_beautify(go_to_companyname_homepage, page_url)
